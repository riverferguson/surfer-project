...
class Surfboard:
    ...
    def shaper(self, shaper):
        if isinstance(shaper, str) and 1 <= len(shaper) <= 15:
            ...
            raise Exception(
                'Shaper must be a string between 1 and 15 characters you barney!')
    ...
    def size(self, size):
        if isinstance(size, str) and 4 <= len(size) <= 14:
            ...
            raise Exception(
                "Size of your board must be a str between 4 and 14...otherwise its not a surfboard")
    ...
    def model(self, model):
        if isinstance(model, str) and 1 <= len(model) <= 15:
            ...
            raise Exception(
                'Model must be a string between 1 and 15 characters hoale')
    ...
    def popularity(self, popularity):
        if isinstance(popularity, int) and 1 <= popularity <= 10:
            ...
            raise Exception(
                'Popularity must be a number between 1 and 10 bro!')
